...
USER_NAME="terraform_user_new"

USER_RESPONSE=$(aws iam get-user --user-name "$USER_NAME")
check_exit_status "Failed to retrieve IAM user."
...
# Create S3 Bucket
S3_BUCKET_NAME="tf-remote-backends-ehb-lars-1243783"
...
sed -e "s/RESOURCE/arn:aws:s3:::$S3_BUCKET_NAME/g" \
    ...
    -e "s|ARN|$USER_ARN|g" "s3-policy.json" > new-policy.json
